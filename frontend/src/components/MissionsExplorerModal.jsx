...
export const MissionsExplorerModal = ({
  ...
  const filteredSections = useMemo(() => {
    ...
    return MISSION_SECTIONS.map(section => {
      ...
      let missions = section.missions.map(id => missionsMap[id]).filter(Boolean);
  ...
  const getCountryCode = (countryStr, agency) => {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {section.missions.map(mission => {
                      ...
                      return (
                        <div
                          key={mission.id}
                          className={`bg-[#0A0F1D]/90 backdrop-blur-md rounded-2xl border p-4 flex flex-col justify-between transition-all duration-200 hover:-translate-y-0.5 ${theme.card}`}
                        >
                          ...
                          <button
                            ...
                            className={`w-full py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${theme.button}`}
                          >
