...
function renderScoreboard() {
  ...
  filtered.forEach((node, idx) => {
    ...
    const statusClass = scoreVal >= 80 ? 'score-status--high' : (scoreVal >= 65 ? 'score-status--moderate' : 'score-status--low');
    const pctClass = scoreVal >= 80 ? '' : (scoreVal >= 65 ? 'score-pct--moderate' : 'score-pct--low');
    ...
    let thumbSrc = `img/landing_sites/thumb_${idx % 5}.jpg`;
    if (node.assets && node.assets.wac_image && node.assets.wac_image.length > 5) {
      thumbSrc = node.assets.wac_image;
    } else if (node.reconnaissance && node.reconnaissance.wac_optical_map) {
      thumbSrc = node.reconnaissance.wac_optical_map;
    }

    const rankNum = node.node_id.replace('N', '');

    row.innerHTML = `
      <div class="score-rank mono">${rankNum}</div>
      <div class="score-thumb" aria-hidden="true">
        <img class="score-thumb-img" src="${thumbSrc}" alt="${node.node_name}" onerror="this.onerror=null; this.src='img/landing_sites/thumb_${idx % 5}.jpg';" />
        <div class="thumb-reticle-overlay"></div>
      </div>
      <div class="score-info">
        <div class="score-name" title="${node.node_name}">${node.node_name}</div>
        <div class="score-coords mono">${node.coordinates.short_formatted || `Lat: ${node.coordinates.latitude.toFixed(2)}° · Lon: ${node.coordinates.longitude.toFixed(2)}° · Alt: +${node.terrain.elevation_m}m`}</div>
      </div>
      <div class="score-meta">
        <div class="score-pct ${pctClass}">${scoreVal}%</div>
...
function updateSiteDetail(node) {
  ...
  // Update Landing Site Photographic Scope Image
  const landImg = $('site-land-img');
  if (landImg) {
    let imgSrc = 'img/landing_sites/site_0.jpg';
    if (node.assets && node.assets.wac_global_recon) {
      imgSrc = node.assets.wac_global_recon;
    } else if (node.reconnaissance && node.reconnaissance.wac_optical_map) {
      imgSrc = node.reconnaissance.wac_optical_map;
    } else if (node.image) {
      imgSrc = node.image;
    }
    landImg.style.transition = 'opacity 0.2s ease, transform 0.35s ease';
    landImg.style.opacity = '0.3';
    landImg.style.transform = 'scale(0.97)';
    const tempImg = new Image();
    tempImg.src = imgSrc;
    tempImg.onload = () => {
      landImg.src = imgSrc;
      landImg.alt = (node.node_name || node.name) + ' Optical Survey';
      landImg.style.opacity = '1';
      landImg.style.transform = 'scale(1)';
    };
    tempImg.onerror = () => {
      landImg.src = 'img/landing_sites/site_0.jpg';
      landImg.style.opacity = '1';
      landImg.style.transform = 'scale(1)';
    };
  }

  // Update HUD alt and survey labels
  const hudSource = document.querySelector('.hud-source-text');
  if (hudSource) hudSource.textContent = (node.node_name || node.name) + ' Optical Survey';
  const hudAlt = document.querySelector('.hud-alt-badge');
  if (hudAlt) {
    const elev = node.terrain && node.terrain.elevation_m !== undefined ? node.terrain.elevation_m : (node.coords ? (node.coords.match(/Alt:\s*([^\s·]+)/) || [])[1] : '4,120m');
    hudAlt.textContent = elev ? (String(elev).startsWith('+') || String(elev).startsWith('-') ? String(elev) : '+' + elev + 'm') : '+4,120m';
  }
...
function renderDeepDiveContent(node) {
  ...
  const nodeIdx = ALL_NODES.findIndex(n => n.node_id === node.node_id);
  const fallbackSiteImg = `img/landing_sites/site_${Math.max(0, nodeIdx) % 5}.jpg`;
  const wacImg = (node.assets && node.assets.wac_image) || fallbackSiteImg;
  const slopeImg = (node.assets && node.assets.slope_map) || 'lunar_node_assets_root/01_Shackleton_Crater/lola_sldem2015_slope.png';

  const hasselPhotos = [
    {
      photo_id: `LROC-${node.node_id}-OPT-01`,
      title: `${node.node_name} Optical Reconnaissance Survey`,
      camera_type: 'LROC Wide Angle Camera (WAC) PDS Level-2',
      hardware_experiments: 'Multispectral Regolith Albedo & Surface Geology',
      features_shown: `High-resolution visual reconnaissance, boulder distribution, and surface morphology at ${node.node_name}`,
      image_url: wacImg
    },
    {
      photo_id: `LOLA-${node.node_id}-SLOPE-02`,
      title: `${node.node_name} LOLA SLDEM2015 Topographic Slope Map`,
      camera_type: 'Lunar Orbiter Laser Altimeter GDR',
      hardware_experiments: '20m Altimetric DEM Slope Survey & Landing Safety',
      features_shown: `Topographic slope steepness, elevation gradient, and landing corridor at ${node.node_name}`,
      image_url: slopeImg
    }
  ];

  const eva = (node.reconnaissance && node.reconnaissance.eva_traverse) || {};
  const evaMapImg = slopeImg || fallbackSiteImg;
  ...
  body.innerHTML = `
    ...
    <div style="padding: 16px; border-radius: 14px; background: #0B1120; border: 1px solid rgba(56, 189, 248, 0.4); display: flex; flex-direction: column; gap: 14px;">
      ...
      <div>
        <div style="font-family: var(--font-mono); font-size: 12px; font-weight: 700; color: #38BDF8; margin-bottom: 8px;">
          1. 70MM HASSELBLAD SURFACE PHOTOGRAPHS &amp; EXPERIMENTS (2 ARCHIVAL FRAMES)
        </div>
        <div class="hassel-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 14px;">
          ${hasselPhotos.map((photo, i) => `
            <div class="hassel-card" style="background: #070D1A; border: 1px solid #162640; border-radius: 10px; overflow: hidden; cursor: pointer;" onclick="openLightbox('${photo.title}', '${photo.image_url}', '${photo.features_shown}')">
              <div class="hassel-img-wrap" style="position: relative; height: 180px; background: #040812; display: flex; align-items: center; justify-content: center; overflow: hidden;">
                <img class="hassel-img" src="${photo.image_url}" alt="${photo.title}" style="width: 100%; height: 100%; object-fit: cover; filter: contrast(1.2) brightness(1.1); transition: transform 0.3s ease;" onerror="this.onerror=null; this.src='${fallbackSiteImg}';" />
                <span class="hassel-frame-tag" style="position: absolute; top: 8px; left: 8px; font-size: 9.5px; font-family: var(--font-mono); font-weight: 700; color: #00E5FF; background: rgba(0, 0, 0, 0.75); border: 1px solid rgba(0, 229, 255, 0.4); padding: 2px 6px; border-radius: 4px;">${photo.photo_id}</span>
                <span class="hassel-expand-icon" style="position: absolute; top: 8px; right: 8px; font-size: 11px; color: #FFF; background: rgba(0, 0, 0, 0.75); border: 1px solid rgba(255, 255, 255, 0.2); padding: 3px 6px; border-radius: 4px;">🔍 Click to Zoom</span>
              </div>
              <div class="hassel-meta" style="padding: 12px; display: flex; flex-direction: column; gap: 6px;">
                <div>
                  <div class="hassel-title" style="font-size: 13px; font-weight: 700; color: #FFF;">${photo.title}</div>
                  <div class="hassel-desc" style="font-size: 11px; color: #94A3B8; margin-top: 3px; line-height: 1.4;">${photo.features_shown}</div>
                </div>
                <div class="hassel-specs" style="font-size: 10.5px; color: #64748B; border-top: 1px solid #142033; padding-top: 6px; margin-top: 4px; display: flex; flex-direction: column; gap: 2px;">
                  <div>Camera: <strong style="color: #CBD5E1;">${photo.camera_type}</strong></div>
                  <div>Experiment: <strong style="color: #38BDF8;">${photo.hardware_experiments}</strong></div>
                </div>
              </div>
            </div>
          `).join('')}
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 12px; margin-top: 8px;">
        <div style="background: #070B14; border: 1px solid #1E293B; border-radius: 10px; padding: 12px;">
          ...
          <div style="height: 140px; border-radius: 8px; overflow: hidden; margin-bottom: 8px; border: 1px solid #162640; cursor: pointer; position: relative;" onclick="openLightbox('${eva.map_title || node.node_name + ' Traverse Corridor'}', '${evaMapImg}', 'Total traverse: ${eva.total_distance_km || 4.8} km')">
            <img src="${evaMapImg}" alt="EVA Traverse Map" style="width: 100%; height: 100%; object-fit: cover; filter: contrast(1.15);" onerror="this.onerror=null; this.src='${fallbackSiteImg}';" />
            <span style="position: absolute; bottom: 6px; right: 6px; font-size: 9px; font-family: var(--font-mono); background: rgba(0,0,0,0.75); color: #F59E0B; padding: 2px 6px; border-radius: 3px;">Corridor Map</span>
          </div>
        ...
        <div style="background: #070B14; border: 1px solid #1E293B; border-radius: 10px; padding: 12px;">
          ...
          <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; margin-top: 4px;">
            <div style="background: #0F172A; padding: 6px 4px; border-radius: 4px; text-align: center; border: 1px solid #1E293B;">
              ...
              <div class="mono" style="font-size: 12px; font-weight: 700; color: #60A5FA;">${node.geology.mineral_composition.plagioclase_pct}%</div>
            </div>
            <div style="background: #0F172A; padding: 6px 4px; border-radius: 4px; text-align: center; border: 1px solid #1E293B;">
              <div style="font-size: 9px; color: #64748B;">Pyroxene</div>
              <div class="mono" style="font-size: 12px; font-weight: 700; color: #34D399;">${node.geology.mineral_composition.pyroxene_pct}%</div>
            </div>
            <div style="background: #0F172A; padding: 6px 4px; border-radius: 4px; text-align: center; border: 1px solid #1E293B;">
              <div style="font-size: 9px; color: #64748B;">Ilmenite TiO₂</div>
              <div class="mono" style="font-size: 12px; font-weight: 700; color: #F59E0B;">${node.geology.mineral_composition.ilmenite_tio2_pct}%</div>
            </div>
            <div style="background: #0F172A; padding: 6px 4px; border-radius: 4px; text-align: center; border: 1px solid #1E293B;">
              <div style="font-size: 9px; color: #64748B;">FeO</div>
              <div class="mono" style="font-size: 12px; font-weight: 700; color: #F87171;">${node.geology.mineral_composition.iron_oxide_feo_pct}%</div>
            </div>
          </div>` : ''}
        </div>
      </div>
    ...
    <!-- SECTION 2: ⛰️ TERRAIN & DEM METRICS -->
    ...
    <div style="padding: 16px; border-radius: 14px; background: #0B1120; border: 1px solid #1E293B;">
      <div style="font-family: var(--font-mono); font-size: 13px; font-weight: 700; color: #38BDF8; margin-bottom: 8px;">
        ⛰️ TERRAIN TOPOGRAPHY &amp; HAZARDS (LOLA 20m SLDEM2015)
      </div>
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; font-size: 11px; color: #94A3B8;">
        <div style="background: #070B14; padding: 10px; border-radius: 8px; border: 1px solid #1E293B;">
          <div style="color: #64748B;">Median Surface Slope</div>
          <div class="mono" style="font-size: 16px; font-weight: 700; color: #34D399; margin-top: 2px;">${node.terrain.slope_deg}°</div>
          <div style="font-size: 10px; color: #34D399; margin-top: 2px;">Safety Rating: ${node.terrain.slope_deg < 10 ? 'EXCELLENT (<10°)' : 'ACCEPTABLE'}</div>
        </div>
        <div style="background: #070B14; padding: 10px; border-radius: 8px; border: 1px solid #1E293B;">
          <div style="color: #64748B;">Absolute Elevation</div>
          <div class="mono" style="font-size: 16px; font-weight: 700; color: #38BDF8; margin-top: 2px;">${node.terrain.elevation_m > 0 ? '+' : ''}${node.terrain.elevation_m} meters</div>
          <div style="font-size: 10px; color: #64748B; margin-top: 2px;">Ref: Lunar Sphere (R=1737.4 km)</div>
        </div>
        <div style="background: #070B14; padding: 10px; border-radius: 8px; border: 1px solid #1E293B;">
          <div style="color: #64748B;">Roughness / Hazard Index</div>
          <div class="mono" style="font-size: 16px; font-weight: 700; color: #F59E0B; margin-top: 2px;">${node.terrain.roughness_index || 0.12}</div>
          <div style="font-size: 10px; color: #F59E0B; margin-top: 2px;">Boulder Field: Low-Moderate</div>
        </div>
      </div>
    </div>` : ''}
    ...
    <div style="padding: 16px; border-radius: 14px; background: #0B1120; border: 1px solid #1E293B;">
      <div style="font-family: var(--font-mono); font-size: 13px; font-weight: 700; color: #60A5FA; margin-bottom: 8px;">
        💧 VOLATILES &amp; WATER ICE DEPOSITS (LEND / Lunar Prospector / M3)
      </div>
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; font-size: 11px; color: #94A3B8;">
        <div style="background: #070B14; padding: 10px; border-radius: 8px; border: 1px solid #1E293B;">
    ...
    <!-- SECTION 5: 🧠 AI/ML MULTI-CRITERIA DECISION MATRIX -->
...
/* ─── FULLSCREEN & IMMERSIVE SIDEBAR MINIMIZATION ─── */
function trigger3DResize() {
  window.dispatchEvent(new Event('resize'));
  if (window.moon3d && typeof window.moon3d.onResize === 'function') {
    window.moon3d.onResize();
  }
}

function animateResizeSeries() {
  trigger3DResize();
  setTimeout(trigger3DResize, 100);
  setTimeout(trigger3DResize, 250);
  setTimeout(trigger3DResize, 400);
}

function enterAppFullscreen() {
  document.body.classList.add('fullscreen-active');
  document.body.classList.remove('sidebar-left-peek', 'sidebar-right-peek');
...
function exitAppFullscreen() {
  document.body.classList.remove('fullscreen-active', 'sidebar-left-peek', 'sidebar-right-peek');
  if (document.fullscreenElement) {
...
function initFullscreen() {
  const btnNav = $('btn-fullscreen');
  const btnExit = $('btn-hud-exit');
  const btnMatrix = $('btn-hud-matrix');
  const btnTelemetry = $('btn-hud-telemetry');
  ...
  if (btnMatrix) {
    btnMatrix.addEventListener('click', (e) => {
      e.preventDefault();
      document.body.classList.toggle('sidebar-left-peek');
      animateResizeSeries();
    });
  }

  if (btnTelemetry) {
    btnTelemetry.addEventListener('click', (e) => {
      e.preventDefault();
      document.body.classList.toggle('sidebar-right-peek');
      animateResizeSeries();
    });
  }
  ...
  // Global Keyboard Shortcuts (Press 'F' to toggle fullscreen, 'Esc' to exit)
...
/* ─── AI CONTROL PANEL SCROLL & PRESETS ─── */
function initAIControlPanel() {
  const scrollContainer = $('sidebar-scroll-container');
  const fadeTop = document.querySelector('.sidebar-fade-top');
  const fadeBottom = document.querySelector('.sidebar-fade-bottom');

  if (scrollContainer && fadeTop && fadeBottom) {
    const handleScroll = () => {
      const st = scrollContainer.scrollTop;
      const maxScroll = scrollContainer.scrollHeight - scrollContainer.clientHeight;
      fadeTop.style.opacity = st > 10 ? '1' : '0';
      fadeBottom.style.opacity = (maxScroll - st > 10) ? '1' : '0';
    };
    handleScroll();
  ...
  formationBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      formationBtns.forEach(b => b.classList.remove('formation-btn--active'));
      btn.classList.add('formation-btn--active');
    });
  });

  const btnAll = $('btn-layers-all');
  const btnNone = $('btn-layers-none');
  const layerChecks = document.querySelectorAll('.layer-check');

  if (btnAll) {
    btnAll.addEventListener('click', () => {
      layerChecks.forEach(ch => ch.checked = true);
    });
  }
  if (btnNone) {
    btnNone.addEventListener('click', () => {
      layerChecks.forEach(ch => ch.checked = false);
    });
  }
}
...
function initRotationControls() {
  const rotBtn = $('btn-rotation-toggle');
  const rotLabel = $('rotation-label');
  const rotIcon = $('rotation-icon');
  ...
  function syncRotationUI(isRotating) {
    if (rotLabel) rotLabel.textContent = isRotating ? 'Stop Rotation' : 'Start Rotation';
    if (rotBtn) {
      rotBtn.setAttribute('aria-pressed', isRotating ? 'true' : 'false');
      rotBtn.classList.toggle('rotation-toggle-btn--paused', !isRotating);
    }
    if (rotIcon) rotIcon.style.animationPlayState = isRotating ? 'running' : 'paused';
    if (btnToggleRotate) {
      btnToggleRotate.textContent = isRotating ? '❚❚' : '▷';
      btnToggleRotate.classList.toggle('globe-ctrl-btn--active', isRotating);
    }
  }

  if (rotBtn) {
    rotBtn.addEventListener('click', () => {
      if (window.moon3d && typeof window.moon3d.toggleRotation === 'function') {
        const rotating = window.moon3d.toggleRotation();
        syncRotationUI(rotating);
      }
    });
  }
  ...
  if (btnReset) {
    btnReset.addEventListener('click', () => {
      if (window.moon3d && typeof window.moon3d.resetView === 'function') {
        window.moon3d.resetView();
        syncRotationUI(true);
      }
    });
  }
  ...
  if (btnZoomIn) {
    btnZoomIn.addEventListener('click', () => {
      if (window.moon3d && typeof window.moon3d.zoomIn === 'function') {
        window.moon3d.zoomIn();
      }
    });
  }
  ...
  if (btnZoomOut) {
    btnZoomOut.addEventListener('click', () => {
      if (window.moon3d && typeof window.moon3d.zoomOut === 'function') {
        window.moon3d.zoomOut();
      }
    });
  }
  ...
  if (btnDebug) {
    btnDebug.addEventListener('click', () => {
      if (window.moon3d && typeof window.moon3d.toggleDebugHUD === 'function') {
        const isOpen = window.moon3d.toggleDebugHUD();
        btnDebug.classList.toggle('globe-ctrl-btn--active', isOpen);
      }
    });
  }
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.code === 'Space') {
      ...
      if (window.moon3d && typeof window.moon3d.toggleDebugHUD === 'function') {
        const isOpen = window.moon3d.toggleDebugHUD();
        if (btnDebug) btnDebug.classList.toggle('globe-ctrl-btn--active', isOpen);
      }
    }
  });
}

/* ─── MODAL CONTROLLERS (TELEMETRY, DOSSIER & DEEP DIVE) ─── */
function initAllModals() {
  const modalTel = $('modal-telemetry');
  const closeTel = $('btn-close-telemetry');
  const btnMissions = $('btn-missions');

  const modalDos = $('modal-dossier');
  const closeDos = $('btn-close-dossier');
  const btnDossier = $('btn-dossier');
  const btnDossierAction = $('btn-dossier-action');

  const modalDeep = $('modal-deep-dive');
  const btnTelemetryAction = $('btn-telemetry');

  // 1. Telemetry Modal
  if (btnMissions && modalTel) {
    btnMissions.addEventListener('click', () => {
      populateTelemetryModalTable();
      modalTel.classList.add('modal-overlay--open');
    });
  }
  if (closeTel && modalTel) {
    closeTel.addEventListener('click', () => modalTel.classList.remove('modal-overlay--open'));
  }

  // 2. Dossier Modal
  if (btnDossier && modalDos) {
    btnDossier.addEventListener('click', () => {
      populateDossierModal();
      modalDos.classList.add('modal-overlay--open');
    });
  }
  if (btnDossierAction && modalDos) {
    btnDossierAction.addEventListener('click', () => {
      populateDossierModal();
      modalDos.classList.add('modal-overlay--open');
    });
  }
  if (closeDos && modalDos) {
    closeDos.addEventListener('click', () => modalDos.classList.remove('modal-overlay--open'));
  }

  // 3. Telemetry Action Button in Selected Site Card -> opens Deep Dive Dashboard
  if (btnTelemetryAction) {
    btnTelemetryAction.addEventListener('click', () => {
      openDeepDiveModal(getSelectedNode());
    });
  }

  // Close modals on outside click
  window.addEventListener('click', (e) => {
    if (modalTel && e.target === modalTel) modalTel.classList.remove('modal-overlay--open');
    if (modalDos && e.target === modalDos) modalDos.classList.remove('modal-overlay--open');
  });

  window.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      if (modalTel) modalTel.classList.remove('modal-overlay--open');
      if (modalDos) modalDos.classList.remove('modal-overlay--open');
    }
  });
}


/* ─── SCIENTIFIC TELEMETRY DASHBOARD (23 NODES) ─── */
function renderTelemetryModalData() {
  const tbody = $('telemetry-table-body');
  if (!tbody) return;
  ...
  (ALL_NODES || []).forEach(node => {
    ...
    tdId.className = 'mono';
    tdId.style.cssText = 'color: #00E5FF; font-weight: 700;';
    tdId.textContent = String(node.node_id || '');
    tr.appendChild(tdId);

    const tdName = document.createElement('td');
    tdName.style.cssText = 'font-weight: 600; color: #FFF;';
    tdName.textContent = String(node.node_name || '');
    tr.appendChild(tdName);

    const tdRegion = document.createElement('td');
    tdRegion.style.color = '#94A3B8';
    tdRegion.textContent = String(node.lunar_region || 'South Pole');
    tr.appendChild(tdRegion);

    const tdCoords = document.createElement('td');
    tdCoords.className = 'mono';
    tdCoords.style.color = '#38BDF8';
    const lat = Number(node.coordinates?.latitude || 0);
    const lon = Number(node.coordinates?.longitude || 0);
    tdCoords.textContent = `${lat > 0 ? '+' : ''}${lat.toFixed(2)}°, ${lon.toFixed(2)}°`;
    tr.appendChild(tdCoords);

    const tdElev = document.createElement('td');
    tdElev.className = 'mono';
    const elev = Number(node.terrain?.elevation_m || 0);
    tdElev.textContent = `${elev > 0 ? '+' : ''}${elev}m`;
    tr.appendChild(tdElev);

    const tdSlope = document.createElement('td');
    tdSlope.className = 'mono';
    tdSlope.textContent = `${node.terrain?.slope_deg ?? 0}°`;
    tr.appendChild(tdSlope);

    const tdHyd = document.createElement('td');
    tdHyd.className = 'mono';
    tdHyd.style.color = '#60A5FA';
    tdHyd.textContent = `${node.water_hydrogen?.hydrogen_abundance_ppm ?? 0} ppm`;
    tr.appendChild(tdHyd);

    const tdTemp = document.createElement('td');
    tdTemp.className = 'mono';
    tdTemp.textContent = `${node.thermal?.minimum_temperature_K ?? 0}K / ${node.thermal?.maximum_temperature_K ?? 0}K`;
    tr.appendChild(tdTemp);

    const tdSun = document.createElement('td');
    tdSun.className = 'mono';
    tdSun.style.color = '#FACC15';
    tdSun.textContent = `${Number(node.illumination?.illumination_percent || 0).toFixed(1)}%`;
    tr.appendChild(tdSun);

    const tdPsr = document.createElement('td');
    tdPsr.style.cssText = `color: ${node.water_hydrogen?.psr_present ? '#34D399' : '#64748B'}; font-weight: 700;`;
    tdPsr.textContent = node.water_hydrogen?.psr_present ? 'YES' : 'NO';
    tr.appendChild(tdPsr);

    const tdScore = document.createElement('td');
    tdScore.className = 'mono';
    tdScore.style.cssText = 'color: #10B981; font-weight: 700;';
    tdScore.textContent = Number(node.ai_suitability?.score || 0).toFixed(1);
...
function populateTelemetryModalTable() {
  renderTelemetryModalData();
}
...
function populateDossierModal() {
  ...
  body.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:14px;color:#e2e8f0;font-size:12px;">
      <div style="display:flex;justify-content:space-between;align-items:center;background:#091224;border:1px solid #162844;border-radius:10px;padding:14px;">
        <div>
          <span style="font-size:10px;font-family:var(--font-mono);color:#00e5ff;text-transform:uppercase;">OFFICIAL MISSION DOSSIER</span>
          <h2 style="font-size:18px;font-weight:700;color:#fff;margin-top:2px;">${node.node_name} (${node.node_id})</h2>
          <div style="font-family:var(--font-mono);font-size:11px;color:#94a3b8;margin-top:4px;">Coordinates: Lat ${node.coordinates.latitude.toFixed(3)}° • Lon ${node.coordinates.longitude.toFixed(3)}° • Elevation: ${node.terrain.elevation_m}m</div>
        </div>
        <div style="text-align:right;">
          <div style="font-size:28px;font-family:var(--font-mono);font-weight:700;color:#10b981;">${node.ai_suitability.score.toFixed(1)}<span style="font-size:14px;color:#64748b;">/100</span></div>
          <span class="site-badge site-badge--high" style="font-size:10px;">${node.ai_suitability.badge}</span>
        </div>
      </div>

      <div style="display:grid;grid-template-columns:repeat(3, 1fr);gap:12px;">
        <div style="background:#070d1a;border:1px solid #142033;border-radius:10px;padding:12px;">
          <div style="color:#64748b;font-size:10px;text-transform:uppercase;font-family:var(--font-mono);">Terrain &amp; Safety</div>
          <div style="font-size:16px;font-weight:700;color:#38bdf8;margin-top:4px;">Slope: ${node.terrain.slope_deg}°</div>
          <div style="color:#94a3b8;font-size:10px;margin-top:2px;">Roughness: ${node.terrain.roughness_index} • Crater: ${node.geology.crater_name || 'N/A'}</div>
        </div>
        <div style="background:#070d1a;border:1px solid #142033;border-radius:10px;padding:12px;">
          <div style="color:#64748b;font-size:10px;text-transform:uppercase;font-family:var(--font-mono);">Water &amp; Resources</div>
          <div style="font-size:16px;font-weight:700;color:#60a5fa;margin-top:4px;">WEH: ${node.water_hydrogen.hydrogen_abundance_ppm} ppm</div>
          <div style="color:#94a3b8;font-size:10px;margin-top:2px;">Ice Traverse: ${node.ai_suitability.ice_traverse_m}m • PSR: ${node.water_hydrogen.psr_present ? 'Confirmed' : 'None'}</div>
        </div>
        <div style="background:#070d1a;border:1px solid #142033;border-radius:10px;padding:12px;">
          <div style="color:#64748b;font-size:10px;text-transform:uppercase;font-family:var(--font-mono);">Solar &amp; Power</div>
          <div style="font-size:16px;font-weight:700;color:#fbbf24;margin-top:4px;">Sunlight: ${node.illumination.illumination_percent.toFixed(1)}%</div>
          <div style="color:#94a3b8;font-size:10px;margin-top:2px;">Continuous: ${(node.illumination.continuous_sunlight_hours/24).toFixed(0)} days • Temp: ${node.thermal.minimum_temperature_K}K-${node.thermal.maximum_temperature_K}K</div>
        </div>
      </div>

      <div style="background:#070d1a;border:1px solid #142033;border-radius:10px;padding:14px;">
        <div style="font-size:11px;font-weight:700;color:#00e5ff;font-family:var(--font-mono);margin-bottom:6px;">SCIENTIFIC RECONNAISSANCE &amp; GEOLOGY SUMMARY</div>
        <div style="line-height:1.6;color:#cbd5e1;font-size:11.5px;">${node.geology.geological_unit_description || 'High-latitude polar highland region featuring continuous low-angle solar illumination and permanently shadowed cold traps with verified volatile hydrogen enhancement.'}</div>
      </div>
    </div>
  `;
}
