...
function initModals() {
  ...
  if (telBtn && telModal) {
    telBtn.addEventListener('click', () => {
      telModal.classList.add('modal-overlay--open');
      renderTelemetryModalData();
    });
  }
  if (closeTel && telModal) {
    closeTel.addEventListener('click', () => {
      telModal.classList.remove('modal-overlay--open');
    });
  }
...
function renderTelemetryModalData() {
  const nodes = (typeof ALL_NODES !== 'undefined' && ALL_NODES.length > 0) ? ALL_NODES : [];
  if (nodes.length === 0) return;

  // Render comparative table
  const tbody = document.querySelector('#telemetry-node-table tbody');
  if (tbody) {
    tbody.innerHTML = nodes.map(n => {
      const c = n.coordinates || {};
      const m = n.metrics || {};
      const elev = c.elevation_m !== undefined ? c.elevation_m + 'm' : (m.terrain ? m.terrain.val + 'm' : 'N/A');
      const slope = m.terrain ? m.terrain.sub : (m.slope ? m.slope.sub : 'N/A');
      const ice = m.ice ? m.ice.sub : 'N/A';
      const temp = m.thermal ? m.thermal.sub : '40K - 220K';
      const illum = m.solar ? m.solar.val + '%' : 'N/A';
      return `<tr>
        <td style="font-weight:700;color:#4B8BF4;">${n.node_id || ''}</td>
        <td><strong>${n.node_name || ''}</strong></td>
        <td style="font-size:11px;opacity:0.8;">${n.folder_name || 'Lunar Target'}</td>
        <td>${c.formatted || 'Polar Region'}</td>
        <td>${elev}</td>
        <td>${slope}</td>
        <td>${ice}</td>
        <td>${temp}</td>
        <td style="color:#34D399;font-weight:600;">${illum}</td>
      </tr>`;
    }).join('');
  }

  // Draw 6 Bar Charts on Canvases
  const chartConfigs = [
    { id: 'chart-elevation', getValue: n => n.coordinates ? (n.coordinates.elevation_m || 3000) : 3000, color: '#4B8BF4' },
    { id: 'chart-slope', getValue: n => n.metrics && n.metrics.terrain ? (parseFloat(n.metrics.terrain.sub) || 5) : 5, color: '#F59E0B' },
    { id: 'chart-hydrogen', getValue: n => n.metrics && n.metrics.ice ? (parseFloat(n.metrics.ice.val) * 10 || 150) : 150, color: '#06B6D4' },
    { id: 'chart-temperature', getValue: n => n.metrics && n.metrics.thermal ? (parseFloat(n.metrics.thermal.val) * 2.5 || 200) : 200, color: '#EF4444' },
    { id: 'chart-illumination', getValue: n => n.metrics && n.metrics.solar ? (n.metrics.solar.val || 80) : 80, color: '#10B981' },
    { id: 'chart-suitability', getValue: n => n.suitability_score || (n.metrics ? n.metrics.solar.val : 85), color: '#8B5CF6' }
  ];
  ...
  chartConfigs.forEach(cfg => {
    ...
    const W = cvs.width = cvs.parentElement.clientWidth || 320;
    const H = cvs.height = 160;
    ctx.clearRect(0, 0, W, H);

    const values = nodes.map(cfg.getValue);
    const maxVal = Math.max(...values, 1);
    const barW = Math.max(3, Math.floor((W - 30) / nodes.length - 2));

    nodes.forEach((n, i) => {
      const val = values[i];
      const barH = Math.max(4, Math.floor((val / maxVal) * (H - 35)));
      const x = 15 + i * (barW + 2);
      const y = H - 20 - barH;
      ...
      ctx.textAlign = 'center';
      if (i % 2 === 0) ctx.fillText(n.node_id || (i+1), x + barW/2, H - 6);
    });
  });
}
