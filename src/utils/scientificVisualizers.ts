...
export function generateRoughnessProfile(roughnessRms: number = 1, _elevationM: number = 0, _craterDiameter: number = 20) {
...
export function generateCraterContours(diameterKm: number = 20, rimDepthM: number = 2000, slopeDeg: number = 5) {
  ...
  const _depth = Math.max(100, Math.min(6000, Number(rimDepthM) || 2000));
  const _slope = Math.max(1, Math.min(25, Number(slopeDeg) || 5));
  ...
  const ryMid = Math.round(ryOuter * 0.52);
