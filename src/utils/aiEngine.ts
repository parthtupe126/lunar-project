import { LunarSite, MissionPriorityWeights, SiteSuitabilityTier } from '../types/lunar';
...
export function calculateSiteScore(
...
): { score: number; tier: SiteSuitabilityTier; confidence: number } {
  const totalWeight =
    weights.waterIce +
    weights.solarEnergy +
    weights.terrain +
  ...
  if (totalWeight === 0) return { score: site.suitabilityScore, tier: site.tier, confidence: site.aiConfidence };
  ...
  let tier: SiteSuitabilityTier = 'POOR';
  if (finalScore >= 88) {
 ...
 */
export function generateAiAssessment(site: LunarSite, weights: MissionPriorityWeights): string {
