...
import { LunarSite, LayerVisibility } from '../../types/lunar';
import { latLonToVector3, LUNAR_MISSIONS, LUNAR_LANDMARKS, LunarMission } from '../../data/lunarSites';
import { 
  Search, 
  RotateCcw, 
  Plus, 
...
function getPhaseName(degrees: number): string {
...
function getEstimatedElevation(lat: number, lon: number): number {
...
export const LunarGlobe3D: React.FC<LunarGlobe3DProps> = ({
  ...
  const [activeTextureMode, setActiveTextureMode] = useState<'lroc_8k' | 'crater_contrast' | 'lola_dem' | 'ice_spectrometry' | 'thermal_diviner'>('lroc_8k');
  const [sunAngle, setSunAngle] = useState<number>(0);
  const [isEvenLighting, setIsEvenLighting] = useState<boolean>(true);
  const [reliefScale, setReliefScale] = useState<number>(2.8);
  const [displacementScale, setDisplacementScale] = useState<number>(0.026);
  const [ambientIntensity, setAmbientIntensity] = useState<number>(0.28);
  const [contrastBoost, setContrastBoost] = useState<number>(1.05);
  const [microDetailScale, setMicroDetailScale] = useState<number>(1.0);
  const [isAutoSpin, setIsAutoSpin] = useState<boolean>(false);
  const [spinSpeed] = useState<number>(0.001);
  const [showMissions, setShowMissions] = useState<boolean>(true);
  const [activeMissionFilter, setActiveMissionFilter] = useState<'all' | 'isro' | 'nasa' | 'spacex' | 'sides'>('all');
  const [selectedMission, setSelectedMission] = useState<LunarMission | null>(null);
  const [isControlsExpanded, setIsControlsExpanded] = useState<boolean>(false);
  const [cursorCoords, setCursorCoords] = useState<{ lat: number; lon: number; elevation: number }>({ lat: -89.20, lon: 15.40, elevation: -3850 });
  const [hoveredObject, setHoveredObject] = useState<{ type: 'mission' | 'site'; data: any } | null>(null);
  ...
  const moonMaterialRef = useRef<THREE.ShaderMaterial | null>(null);
  const globeGroupRef = useRef<THREE.Group | null>(null);
  const markersGroupRef = useRef<THREE.Group | null>(null);
  const missionsGroupRef = useRef<THREE.Group | null>(null);
  ...
  const flyToCoords = (latDeg: number, lonDeg: number, distance: number = 2.4) => {
  ...
  const handleSelectSiteInternal = (site: LunarSite) => {
    setSelectedMission(null);
    onSelectSite(site);
    flyToCoords(site.latitude, site.longitude, 2.2);
  };

  const handleSelectMissionInternal = (mission: LunarMission) => {
              ...
              <div
                ...
                onClick={() => {
                  if ('isMission' in item && item.isMission) {
                    handleSelectMissionInternal((item as any).missionData);
                  } else {
                    handleSelectSiteInternal(item as LunarSite);
                  }
                  setSearchQuery('');
                }}
              ...
              >
                <div>
                  <div className="text-xs font-mono font-bold text-white flex items-center gap-1.5">
                    {'isMission' in item && item.isMission ? <Rocket className="w-3 h-3 text-orange-400" /> : <MapPin className="w-3 h-3 text-cyan-400" />}
                    <span>{item.name}</span>
                  </div>
      ...
      <div className="absolute bottom-4 left-4 right-4 z-20 pointer-events-none flex flex-col gap-2">
        ...
        {selectedMission && (
          <div className="pointer-events-auto bg-[#070B14]/95 backdrop-blur-xl border border-cyan-500/40 p-4 rounded-2xl shadow-2xl max-w-xl animate-in fade-in slide-in-from-bottom-2 duration-300 flex flex-col gap-3">
            ...
            {/* Clickable Full Scientific Telemetry Page Opener */}
            <button
              onClick={() => {
                soundManager.playSelect();
